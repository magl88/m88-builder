...
export const watch = () => {
	app.gulp
		.watch(app.path.src.assetsFolder + "svg-src/**/*")
	...
	app.gulp
		.watch(app.path.srcFolder + "**/*.html")
		.on("all", app.gulp.series(tasks.html, tasks.webServerCreate.reload));
	app.gulp
		.watch(app.path.srcFolder + "**/*.php")
		.on(
			"all",
			app.gulp.series(
				process.env.BUILD_TYPE == "php" ? tasks.php : tasks.wordpress,
				tasks.webServerCreate.reload,
			),
		);
	app.gulp
