...
export const wpFiles = () => {
	return app.gulp
		...
		.pipe(
			replace(/@sprite/g, "<?php echo get_template_directory_uri();?>/assets/sprite/sprite.svg"),
		)
...
export const wpInc = () => {
	return app.gulp
		...
		.pipe(replace(/@css\//g, "<?php echo get_template_directory_uri();?>/css/"))
		.pipe(replace(/@js\//g, "<?php echo get_template_directory_uri();?>/js/"))
		.pipe(app.gulp.dest(app.path.buildFolder + "inc"));
};

export const wpLanguages = () => {
	return app.gulp
		.src([app.path.srcFolder + "languages/**/*"])
		.pipe(app.gulp.dest(app.path.buildFolder));
};

export const wpScreenshot = () => {
	return app.gulp
		.src([app.path.srcFolder + "screenshot.png"])
		.pipe(app.gulp.dest(app.path.buildFolder));
};
...
export const wpStyle = () => {
	return app.gulp
		...
		.pipe(replace("Theme m88", "Theme " + process.env.PROJECT_NAME))
		.pipe(app.gulp.dest(app.path.buildFolder));
};
